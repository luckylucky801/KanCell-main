...
class PredictionModel(nn.Module):
    def __init__(self, input_dims, latent_dims, hidden_dims, celltype_dims, dropout, num_heads=8):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.LayerNorm(hidden_dims),
            # MultiHeadAttention(hidden_dims, num_heads),
    ...
    def initialize_weights(self):
        ...
        for layer in self.pred:
            if isinstance(layer, KAN.KANLinear):
...
class KanCell_lModel():
    def __init__(
        ...
        use_gpu=True,
        seed=42,
        # noise_stddev=0.1  # Add noise_stddev parameter
    ):
        ...
        self.sm_lr = sm_lr
        self.st_lr = st_lr
        # self.noise_stddev = noise_stddev  # Add noise_stddev attribute
        self.init_model()
        self.st_data = None
        self.sm_data = None
        self.sm_labels = None
    ...
    def init_model(self):
        self.model = PredictionModel(
            ...
            len(self.clusters),
            0.8,
            # self.noise_stddev  # Pass noise_stddev to the PredictionModel
        ).to(self.device)
    ...
    def train(
        ...
        if save_path is None:
            save_path = os.path.join(tempfile.gettempdir() ,'KanCell_l_models_'+strftime("%Y%m%d%H%M%S",localtime()))
